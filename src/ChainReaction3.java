...
public class ChainReaction3 {
	...
	public static void main(String[] args) {
		...
		ChainReaction3 chainReaction = new ChainReaction3();
	...
	public void fillGrid(int row, int col, char element, int type) {
		...
		if (elementGrid[row][col] == 0) { // if empty
			...
			if (type == 0 && massGrid[row][col] + 1 < getCriticalMass(row, col)) {
				massGrid[row][col]++;
			} 
			else if (type == 1 && massGrid[row][col] + 1 <= getCriticalMass(row, col)) {
				massGrid[row][col]++;
			}
			else {
				if (type == 0) {
					elementGrid[row][col] = 0;
					massGrid[row][col] = 0;
					fillGrid(row+1, col, element, type+1);
					fillGrid(row, col+1, element, type+1);
					fillGrid(row-1, col, element, type+1);
					fillGrid(row, col-1, element, type+1);
				}				
			}
			
		} else if (type == 1) { // if different element
			elementGrid[row][col] = element;
			massGrid[row][col]++;
		}
